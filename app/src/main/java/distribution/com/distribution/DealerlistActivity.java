    ...
    private void hist() {
        class UserLogin2 extends AsyncTask<Void, Void, String> {
                    ...
                    JSONObject obj = new JSONObject(s);
                            ...
                            for (int i = 0; i < userJson.length(); i++) {
                                ...
                                User7 item = new User7(
                                        count.getString("id"),
                                        count.getString("user_name"),
                                        count.getString("name"),
                                        count.getString("phone"),
                                        count.getString("address"),
                                        count.getString("user_username"),
                                        count.getString("created_at"),
                                        count.getString("user_email"),
                                        count.getString("user_address"),
                                        count.getString("user_phone")
                            ...
                            adapter = new MyAdapter6(listItems, getApplicationContext());
                            recyclerView.setAdapter(adapter);
            ...
            protected String doInBackground(Void... voids) {
                User user = SharedPrefManager.getInstance(DealerlistActivity.this).getUser();
                ...
                return requestHandler.sendPostRequest(URLs.URL_DATADEALERTABLE, params);
